...
class ModelsSimplification:
     @staticmethod
     def decile_group_sum_rating_students_by_semestr(merged_df, term_number: int):
        """Дециль в группе сумма рейтингов студента по всем дисциплинам номера(term_number) семестра
        ...
        student_grouped_1_sem = df_first_semester.groupby('student_id')['Rating'].sum().reset_index()
        # сортировка по сумме рейтингов и вычисление децилей
        student_grouped_1_sem = student_grouped_1_sem.sort_values(by='Rating', ascending=False)
        student_grouped_1_sem[f'DecileSumRatingStudent{term_number}Sem'] = pd.qcut(student_grouped_1_sem['Rating'], 5, labels=False, duplicates='drop') + 1

        # объединяем результаты с исходным DataFrame
        merged_df = pd.merge(merged_df, student_grouped_1_sem[['student_id', f'DecileSumRatingStudent{term_number}Sem']], on='student_id', how='inner')
        return merged_df
     
     @staticmethod
     def decile_median_rating_disciplin_check_type_by_semestr(merged_df, check_type: int, term_number: int):
        """Дециль медианного рейтинга дисциплины зачета/экзамена(check_type) номера(term_number) семестра
        ...
        credit_decile_ratings = pd.qcut(credit_median_ratings, 5, labels=False, duplicates='drop') + 1
